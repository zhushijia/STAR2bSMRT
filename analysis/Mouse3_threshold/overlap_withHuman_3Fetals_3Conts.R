...
for(threshold in c(1:10))
{
  ...
  sampleNames = c("3' Del 1","3' Del 2","Control 1","Control 2","Control 3","Adult 1","Adult 2","Adult 3","Fetal 1","Fetal 2","Fetal 3") #
  ...
  Group = rep( c("Case","Control","Adult","Fetal","Mouse") , c(2,3,3,3,1) )
  
  #####################################################################################
  ##########      get fraction and from fraction to tag since mouse and human coordination different
  #####################################################################################
  
  humanGroup = list( Case=c(1:2), Control=c(3:5), Adult=c(6:8), Fetal=c(9:11)  )
  ...
  humanmouseExps0 = lapply( 3:12, function(i) {
    if( Group[i]=="Mouse")
    { 
  ...
  exps = t(apply(humanmouseExps,1,function(x) tapply(x,Group[3:12],mean) ) )
  ...
  setwd("/sc/orga/projects/schzrnas/sjzhu/Project/NRXN/result/STAR2bSMRT/pipelineNew_testParameters/comparisonResult/adjustNCjunc_TRUE_fixedMatchedLS_FALSE_useSJout_FALSE_fuzzyMatch_100/RevisionThresholdTesting_3Cont3Fetal")
  ...
  tagList2 = Tags[3:5]
  tagAll2 = unique(do.call(c, tagList2))
  indexList2 <- lapply(tagList2, function(x) which(tagAll2 %in% x))
  names(indexList2) = names(tagList2)
  setwd("/sc/orga/projects/schzrnas/sjzhu/Project/NRXN/result/STAR2bSMRT/pipelineNew_testParameters/comparisonResult/adjustNCjunc_TRUE_fixedMatchedLS_FALSE_useSJout_FALSE_fuzzyMatch_100/RevisionThresholdTesting_3Cont3Fetal")
  filename = paste0("vennDiagram_Mouse_Adult_Fetal_threshold_",threshold,".png")
  venn.diagram(indexList2, filename = filename, height = 2000, 
               width = 2000, fill = 1:length(tagList2), alpha = 0.4, 
               imagetype = "png")
  ...
  names(indexList2) = names(tagList2)
  setwd("/sc/orga/projects/schzrnas/sjzhu/Project/NRXN/result/STAR2bSMRT/pipelineNew_testParameters/comparisonResult/adjustNCjunc_TRUE_fixedMatchedLS_FALSE_useSJout_FALSE_fuzzyMatch_100/RevisionThresholdTesting_3Cont3Fetal")
  filename = paste0("vennDiagram_Cont_Adult_Fetal_threshold_",threshold,".png")
  venn.diagram(indexList2, filename = filename, height = 2000, 
               width = 2000, fill = 1:length(tagList2), alpha = 0.4, 
               imagetype = "png")
