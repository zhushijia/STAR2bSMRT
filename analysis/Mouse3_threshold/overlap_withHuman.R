...
Group = rep( c("Case","Control","Adult","Fetal","Mouse") , c(2,2,3,1,1) )

#####################################################################################
##########      get fraction and from fraction to tag since mouse and human coordination different
#####################################################################################

humanGroup = list( Case=c(1:2), Control=c(3:4), Adult=c(5:7), Fetal=c(8)  )
...
humanmouseExps0 = lapply( 3:9, function(i) {
  if( Group[i]=="Mouse")
  { 
...
exps = t(apply(humanmouseExps,1,function(x) tapply(x,Group[3:9],mean) ) )
...
filename = paste0("vennDiagram_withMouse_threshold_",threshold,".png")
...
pdf( paste0("Scatterplot_exp_mouse_human_threshold_",threshold,".pdf") )
