...
pdf("Exlucsion_FC_KCLvsPSB_byJunc_withError.pdf")
barx <- barplot(PSI_ratio,col=rep(c('grey','darkred','black'),11),las=3,ylab='Exclusion fold change',ylim=c(0,1.9))
arrows(barx , PSI_ratio+PSI_error, barx , PSI_ratio, angle=90, code=3, length=0.05)
dev.off()

## for checking SS4 pvalue
i = 9
wilcox.test( c(ratio_cont_2607[i],ratio_cont_553[i]) , c(ratio_cont_2607[-i],ratio_cont_553[-i]) , 'greater' )
