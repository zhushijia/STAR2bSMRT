...
heatmapMannualColor = function(X)
{
  ...
  gg <- gg + scale_fill_manual(values=c("darkgrey","white","darkorange","darkgreen"))
  gg <- gg + labs(x="", y="")
  gg <- gg + theme_bw()
...
library(STAR2bSMRT,lib.loc="/hpc/users/xzhus01/schzrnas/sjzhu/Project/NRXN/code/STAR2bSMRT/githubClone5/setup")
...
drawSchematics = function( case_ind, cont_ind, fileName , threshold  )
{
  # parai=parameters[6] 
...
#drawSchematics( case_ind = c(3,4) , cont_ind = c(9) , fileName="hiPSCcont.postmortemFetal_orderedExp.pdf")
#drawSchematics( case_ind = c(3,4) , cont_ind = c(6:8) , fileName="hiPSCcont.postmortemAdult_orderedExp.pdf")
drawSchematics( case_ind = c(3,4) , cont_ind = c(6:11) , fileName="hiPSCcont_postmortemFetalAdult_orderedExp", threshold=7 )
