...
#'
#' @examples
getLRinfo2 = function( alignments , phqv=NULL , outputDir , chrom , s , e )
{
  ...
  if( !is.null(phqv) )
  {
	...
	LRjunc = foreach( i = 1:length(LRread) ) %dopar%
	{
		...
		data.frame(count=as.integer(uniJuncCount),chr=names(LRread)[i],start=start,end=end,stringsAsFactors=F)
	}
